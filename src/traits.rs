...
pub fn resolve(tree: &impl TreeStore, cache: &impl CacheStore, pid: u32) -> Option<ProcInfo> {
    // Try cache first
...
pub fn find_by_cmd(tree: &impl TreeStore, cache: &impl CacheStore, target_cmd: &str) -> Vec<u32> {
    tree.all_pids()
        .into_iter()
        .filter(|&pid| {
            let cmd = tree
